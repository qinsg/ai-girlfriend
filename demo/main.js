...
const GATE_DEFAULT_DB = GATE_OFF_DB; // first-run default: pure mic passthrough
...
function loadSettings() {
  return {
    ...
    character: localStorage.getItem(STORAGE_KEYS.character) || "xiaoman",
 ...
 * disabled gate (GATE_DEFAULT_DB) when the user hasn't set one yet. */
...
function saveSettings(s) {
  ...
  localStorage.setItem(STORAGE_KEYS.character, s.character || "");
...
function loadTools() {
  try {
    ...
    // Companion tools are opt-in. The local voice path works without external
    // search credentials or webcam permission.
    return {
...
/** @type {HTMLButtonElement} */
const testSpeakerBtn = $("#test-speaker");
...
async function loadCharacterPresets() {
  try {
    ...
    characterPresets = await response.json();
    inputCharacter.replaceChildren();
    for (const [id, preset] of Object.entries(characterPresets)) {
      const option = document.createElement("option");
      option.value = id;
      option.textContent = preset.label || id;
      inputCharacter.append(option);
      if (preset.voice && !Array.from(inputVoice.options).some((voice) => voice.value === preset.voice)) {
        const voiceOption = document.createElement("option");
        voiceOption.value = preset.voice;
        voiceOption.textContent = preset.voice;
        inputVoice.append(voiceOption);
      }
    }
    if (!characterPresets[settings.character]) settings.character = Object.keys(characterPresets)[0] || "";
    inputCharacter.value = settings.character;
    if (!localStorage.getItem(STORAGE_KEYS.voice) && characterPresets[settings.character]) {
      settings.voice = characterPresets[settings.character].voice || DEFAULT_VOICE;
...
inputCharacter.addEventListener("change", () => {
  const preset = characterPresets[inputCharacter.value];
  if (!preset) return;
  inputVoice.value = preset.voice || DEFAULT_VOICE;
  inputInstructions.value = preset.instructions || DEFAULT_INSTRUCTIONS;
});
...
function createResumedAudioContext() {
  try {
    ...
    // Start a one-sample silent source while the click is still active. This
    // survives the microphone permission dialog on browsers that otherwise
    // suspend Web Audio before the first TTS packet arrives.
...
function readSettingsFromForm() {
  return {
    ...
    character: inputCharacter.value || settings.character,
...
circleBtn.addEventListener("click", async () => {
  try {
    if (currentState === "idle" || currentState === "error") {
      ...
      await doStart();
    } else if (LIVE_STATES.has(currentState) && client) {
      const running = await client.resumeAudio(true);
      setCaption(running ? "声音已启用" : "浏览器仍在阻止声音，请在设置中测试扬声器", running ? "" : "error");
    }
  } catch (err) {
    await handleStartError(err);
  }
...
testSpeakerBtn.addEventListener("click", async () => {
  if (client && LIVE_STATES.has(currentState)) {
    const running = await client.resumeAudio(true);
    audioOutputHint.textContent = running
      ? "已播放测试音。如果仍听不到，请切换上面的扬声器。"
      : "浏览器阻止了播放。请再点一次测试扬声器。";
    audioOutputHint.classList.toggle("error", !running);
    return;
  }
  const ctx = createResumedAudioContext();
  if (!ctx) {
    audioOutputHint.textContent = "浏览器无法创建音频输出。";
    audioOutputHint.classList.add("error");
    return;
  }
  try {
    if (settings.audioOutputId && typeof /** @type {any} */ (ctx).setSinkId === "function") {
      await /** @type {any} */ (ctx).setSinkId(settings.audioOutputId);
    }
    ...
    audioOutputHint.textContent = "已播放测试音。如果仍听不到，请切换上面的扬声器。";
    audioOutputHint.classList.remove("error");
  } catch (error) {
    void ctx.close().catch(() => {});
    audioOutputHint.textContent = "测试音播放失败，请换用 Chrome 或 Edge。";
    audioOutputHint.classList.add("error");
  }
});
...
/** On this Mac, avoid a system default that may point at Teams/WeMeet. Once
 * permission reveals labels, prefer the physical built-in mic unless the user
 * has already chosen a device. */
async function preferPhysicalMic() {
  try {
    ...
    if (!builtIn) return;
    settings.audioInputId = builtIn.deviceId;
    localStorage.setItem(STORAGE_KEYS.audioInputId, builtIn.deviceId);
    inputAudioInput.value = builtIn.deviceId;
    console.info(`[audio] using built-in microphone: ${builtIn.label}`);
  } catch (error) {
    console.warn("[audio] could not choose built-in microphone:", error);
  }
}
...
async function doStart(audioContext = null) {
  ...
  c.addEventListener("audio-state", (e) => {
    ...
    if (detail.receivedBytes && detail.state !== "running") {
      setCaption("语音已生成，但浏览器拦截了播放。点圆球启用声音。", "error");
    }
  });
