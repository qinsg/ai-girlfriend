...
export class S2sRealtimeClient extends EventTarget {
  ...
  async _setupAudio() {
    ...
    if (this.options.transport === "websocket") {
      ...
      const capture = new AudioWorkletNode(ctx, "mic-capture", {
        numberOfInputs: 1,
        // Keep one silent output connected to destination. Chromium/WebKit may
        // stop pulling a zero-output recorder after setup, which leaves the
        // socket connected but sends no microphone PCM.
        numberOfOutputs: 1,
      ...
      const captureKeepAlive = ctx.createGain();
      captureKeepAlive.gain.value = 0;
      capture.connect(captureKeepAlive).connect(ctx.destination);
      this._captureNode = capture;
      this._captureKeepAlive = captureKeepAlive;
      ...
      playback.port.onmessage = (event) => {
        if (event.data?.kind !== "stats") return;
        this._playbackFrames = Number(event.data.played) || 0;
        this.dispatchEvent(new CustomEvent("audio-state", { detail: {
          state: ctx.state,
          queuedMs: Number(event.data.queuedMs) || 0,
          playedFrames: this._playbackFrames,
        } }));
      };
  ...
  _onAudio(event) {
    ...
    // Some browsers suspend Web Audio again after the microphone permission
  ...
  /** Resume browser playback from a click/tap. Optionally play a short test tone. */
  async resumeAudio(testTone = false) {
    const ctx = this._ctx;
    if (!ctx || ctx.state === "closed") return false;
    try {
      if (ctx.state !== "running") await ctx.resume();
      if (testTone && ctx.state === "running") {
